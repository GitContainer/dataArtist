    ...
    * perspective
    ...
    def activate(self):   
        if seriesResistanceMapping is None:
            raise Exception('proprietary module [%s] not found' %'PROimgProcessor')
    ...
    def _process(self):  
        ...
        #checks:
        assert len(img) in (2,3),'need 2EL  OR 2EL and 1BG images in the display'
        ...
        if p.value():
            ...
            try:
                expTime1, voltage1, current1, temperature1 = paramFromFilename(f[0].basename())
                expTime2, voltage2, current2, temperature2 = paramFromFilename(f[1].basename())
            except Exception as e:
                raise Exception('Cannot extract parameters frmo file name: %s' %e)
        else:
