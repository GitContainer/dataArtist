...
from pyqtgraph_karl.parametertree import Parameter, ParameterTree
...
class DisplayDock(Dock):
    ...
    * creating PREFERENCES in INFORMATION about the input
    ...
    def __init__(self, number, workspace, origin=None, index=None,
        ...
        # INIT WIDGET
        self.changeWidget(widgetList.getWidget())
    ...
    def _limitLayers(self, _param, val):
    ...
    def _pTitleFromLayerChanged(self, _param, val):
    ...
    def _setWidgetTitle(self, _param, title):
        if not title:
            title = None
    ...
    def _extractInfoFromOrigin(self, origin, index,
        ...
        if origin:
            ...
            if names is None:
                names = list(origin.filenames)
                # substitude Nones with child names:
                for n, (c, na) in enumerate(zip(ch, names)):
                    if na is None:
                        names[n] = c.name()
    ...
    def addLayer(self, data=None, filename=None, label=None, origin=None,
        ...
        if self.widget.moveLayerToNewImage is not None:
            ...
            # LIMIT LAYERS
            if (self.pLimitLayers.value() and
                    len(self.stack.childs) > self.pMaxLayers.value()):
    ...
    def backupChangedLayer(
            self, backup=None, changes=None, index=None, **_kwargs):
        # UNDO/REDO
...
class _StackParameter(GroupParameter):
    ...
    def restoreState(self, state, **_kwargs):
        ...
        if 'children' in state:
            ...
            # REBUILD STACK:
            for name, sub in state['children'].items():
                number = sub['default']
                label = sub['layername']
                info = sub['children']['Info']['value']
                fname = sub['filename']
                changes = sub['children']['Changes']['value']
                self.buildLayer(fname, label, name, None, info=info,
                                changes=changes, number=number)
#         GroupParameter.restoreState(self, state, **kwargs)
            self.sigChildAdded.connect(self._fnInsertRemovedLayer)

    def buildLayer(self, fname, label, name, data, origin=None, index=None,
                   info=None, changes=None, number=None):
        '''
        ...
        * a layer changed field
        ...
        if self.display.reader is not None:
            ...
            aFile.triggered.connect(lambda _checked, i=len(self.childs):
                                    self.display.changeLayerFile(i))
            menu_entries.append(aFile)
        # CREATE AND ADD COPY-LAYER-TO OPTION TO PARAMETER:
        if number is None:
            number = self.display.axes.stackAxis.getNextStackValue(
    ...
    def buildCopyToDisplayMenu(self, menuCopy, paramFile, method):
        ...
        menuCopy.addAction('NEW').triggered.connect(
            lambda _checked, paramFile=paramFile, m=m:
        ...
        for d in self.display.workspace.displays():
            if (d != self.display and
                    d.widget.__class__ == self.display.widget.__class__):
                menuCopy.addAction(d.name()).triggered.connect(
                    lambda _checked, paramFile=paramFile, d=d, m=m:
