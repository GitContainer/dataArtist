...
class PlotReader(ReaderBase):
    ...
    def open(self, filename):
        ...
        if stop_at_line:
            ...
            if not (self._lastFilename ==
                    filename and self._lastModified == modified):
        ...
        with open(filename, 'r') as f:
            ...
            if filter_columns:
                c = [
                    ch.value() for ch in prefs.pFilterColumns.children()[
                        1:]][
                    :n_col]
            ...
            if n_col == 0:
                ...
                shape = (shape, n_col)
            data = np.empty(
                shape=shape, dtype=prefs.dtypes[
                    prefs.pDType.value()])
    ...
    def _findSeparator(self, f):
        ...
        sep = list(
            separators.keys())[
            numberOfSeparator.index(
                max(numberOfSeparator))]
